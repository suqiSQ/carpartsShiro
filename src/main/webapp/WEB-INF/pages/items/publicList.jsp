...
<link rel="stylesheet" type="text/css" href="${ctx}/static/jBox/Skins-maitian/maitian-simple/jbox.css"/>
<script src="${ctx}/static/treeTable/jquery.treeTable.js"
	type="text/javascript"></script>
<script src="${ctx}/static/zTree/js/jquery.ztree.core-3.5.js"
	type="text/javascript"></script>
<link href="${ctx}/static/zTree/css/zTreeStyle/zTreeStyle.css"
	rel="stylesheet" type="text/css" />
<link href="${ctx}/static/treeTable/themes/vsStyle/treeTable.css"
	rel="stylesheet" type="text/css" />
<SCRIPT type="text/javascript">
        ...
        //数据源 使用c标签循环拼接json
        var zNodes =[
            <c:forEach items="${brandList}" var="menu">{id:"${menu.id}", pId:"${menu.parentId}", name:"${menu.name}"},
            </c:forEach>]
	...
	<div class="bg_color1" id="idett">
		...
		<div class="pagebody1" style="height:700px">
			...
			<div class="wid">
				...
				<div class="pagebody1_right">
					<form id="searchPublicForm" name="searchPublicForm"
						...
						action="${ctx}/items/publicList.shtml" method="post">
					...
					<c:forEach var="sale" items="${page.list}">
						<ul class="qps">
							<a href="#" onclick="productgroupedit('${sale.id}');"
								class="fban">
								<div class="goods">
									<img src="${ctx}/images/${sale.picurl}" />
									<div class="goods1">
										<h3>
			...
			<ul class="page">
					    ...
					    <a href="${ctx}/items/publicList.shtml?pageNo=${pageSize}&pageSize=8&title=${items.title}&content=${items.content}&brandId=${items.brandId}&brandName=${brandName}&partId=${items.partId}" style="background: #C30D23 none repeat scroll 0% 0%;color:#ffffff;">${pageSize}</a>
					     </c:when>
					   <c:otherwise>
					   <a href="${ctx}/items/publicList.shtml?pageNo=${pageSize}&pageSize=8&title=${items.title}&content=${items.content}&brandId=${items.brandId}&brandName=${brandName}&partId=${items.partId}" >${pageSize}</a>
					   </c:otherwise>
					   </c:choose>
