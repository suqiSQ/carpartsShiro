...
<script src="${ctx}/js/jquery.min2.js" type="text/javascript" charset="UTF-8"></script>
...
$(document).ready(function(){
	...
	$("#loginName").formValidator({onshow:"请输入用户名",onfocus:"6-20位字母、数字、下划线组成",oncorrect:"该用户名可以注册"}).inputValidator({min:6,max:20,onerror:"用户名长度不满足要求"}).regexValidator({regexp:"username",datatype:"enum",onerror:"用户名格式不正确"})
	    .ajaxValidator({
	    type : "post",
		url : "${ctx}/login/checkLoginName.shtml",
	...
	.ajaxValidator({
	type : "post",
	url : "${ctx}/login/checkPhone.shtml",
	...
	.ajaxValidator({
	type : "post",
	url : "${ctx}/login/checkEmail.shtml",
    ...
    .ajaxValidator({
    type : "post",
	url : "${ctx}/login/checkCompanyname.shtml",
...
function getProvince() {
	$.ajax( {
		type :'post',
		url :"${ctx}/login/getProvince.shtml",
		dataType :'json',
...
function getCity() {
	...
	if (object.val() != 0) {
		$.ajax( {
			type :'post',
			url :"${ctx}/login/getCity.shtml?id="+object.val(),
			dataType :'json',
			success : function(result) {
				$.each(result, function(entryIndex, entry) {
					var html = "<option value='" + entry["id"] + "'>" + entry["name"] + "</option>";
...
<body  onload="getProvince()">
	...
	<div class="wid top1"><img src="${ctx}/static/parts/images/logo1.jpg" /></div>
    <div class="zhuce">
    	<div class="wid">
    	<form  method="post" id="registerForm" action="${ctx}/login/register.shtml">
        	<div class="zhuce_wid">
